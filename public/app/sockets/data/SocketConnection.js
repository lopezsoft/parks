 ...
 *
 */
Ext.define('Admin.sockets.data.SocketConnection', {
	mixins: {
		observable: 'Ext.mixin.Observable'
	},

	requires: [
		'Admin.sockets.data.request.Socket',
	...
	request: function(options) {
		...
		if (me.fireEvent('beforerequest', me, options) !== false) {
			...
			return request.start(requestOptions);
		}
	...
	setOptions: function(options, scope) {
		var me = this,
			...
			extraParams = proxy ? proxy.getExtraParams() : options.extraParams || me.getExtraParams() || {};
		...
		return {
			data				: data,
			url	                : url,
			host				: proxy ? proxy.host : options.host || me.getHost(),
			...
			timeout             : proxy ? proxy.wsTimeout : me.getTimeout(),
