...
class MasterModel
{
    ...
    function checkClosingSession(Request $request, $id) {
        ...
        $result = false;
        if ($query) {
            $result = ($query->opening_date == $date) ? false : true;
        }
    ...
    public function setImageProd($file = null, $id = 0)
    {
        if (!is_null($file) and $id > 0) {
            ...
            $urlM = "products/" . $id;
            Storage::putFileAs('public/'.$urlM, new File($file), $fielName);
            $mime = Storage::mimeType('public/'.$urlM . '/' . $fielName);
